...
class Challenge2 extends AnyWordSpec with Matchers {
  def checkDesigns(lines: List[String]): Long = {
    ...
    def check(rem: String): Long =
      if (rem.isEmpty) 1
      else if (memo.contains(rem)) memo(rem)
      else
        patterns
          .filter(rem.startsWith)
          .map(_.length)
          .map(rem.drop)
          .map { p =>
            val res = check(p)
    ...
    designs.tail.map { p => memo.clear(); check(p) }.sum
  }
  ...
  "Day #19 Challenge #2" should {
    "work as expected #1" in {
      ...
      checkDesigns(input) shouldBe 16
    }
    ...
    "work as expected #4" in {
      ...
      checkDesigns(input) shouldBe 616957151871345L
    }
  }
}
